...
export default function App(){
  ...
  const playPreview=useCallback(async(name)=>{
    ...
    const strategies=[
      // 1. Deezer API (has CORS support)
      async()=>{
        const r=await fetch(`https://api.deezer.com/search?q=${encodeURIComponent(name)}&limit=1`);
      ...
      async()=>{
        const r=await fetch(`https://corsproxy.io/?${encodeURIComponent(`https://itunes.apple.com/search?term=${encodeURIComponent(name)}&media=music&limit=1`)}`);
        const d=await r.json();
      ...
      async()=>{
        const target=encodeURIComponent(`https://itunes.apple.com/search?term=${encodeURIComponent(name)}&media=music&limit=1`);
  ...
  const venues=useMemo(()=>{
    ...
    return Object.entries(vm).sort((a,b)=>{
  ...
  return(
    <div style={{fontFamily:"'IBM Plex Mono',monospace",background:"#08080d",color:"#d4d4d4",minHeight:"100vh"}}
      ...
      <div style={{padding:"16px 20px",maxWidth:1500,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",alignItems:"baseline",gap:10,marginBottom:4,flexWrap:"wrap"}}>
          <h1 style={{fontSize:28,fontWeight:700,margin:0,background:"linear-gradient(90deg,#E63946,#FFB703,#06D6A0,#8338EC,#FF006E)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>
            SXSW 2026
          </h1>
          <span style={{fontSize:14,color:"#555"}}>OFFICIAL SCHEDULE — GANTT BY VENUE</span>
        ...
        <p style={{fontSize:11,color:"#4a4a55",margin:"0 0 12px",lineHeight:1.4}}>
          ...
          {DAYS.map(d=>{
            ...
            return(
              <button key={d} onClick={()=>setDay(d)} style={{
                padding:"8px 16px",borderRadius:6,cursor:"pointer",fontSize:13,fontWeight:day===d?700:500,
              ...
              }}>{DAY_LABEL[d]} <span style={{opacity:.5}}>({cnt})</span></button>
            );
          })}
          <div style={{position:"relative",marginLeft:8}}>
            <input placeholder="Search artist, venue, presenter..." value={search} onChange={e=>setSearch(e.target.value)}
              style={{padding:"8px 12px",paddingRight:search?30:12,borderRadius:6,border:"1px solid #222",background:"#13131d",color:"#aaa",
              fontSize:13,fontFamily:"inherit",width:280,outline:"none"}}/>
        ...
        <div style={{display:"flex",gap:4,marginBottom:6,flexWrap:"wrap",alignItems:"center"}}>
          ...
          {Object.entries(G).filter(([g])=>genreCounts[g]).sort((a,b)=>(genreCounts[b[0]]||0)-(genreCounts[a[0]]||0)).map(([genre,color])=>(
            <button key={genre} onClick={()=>{const next=new Set(gf);if(next.has(genre))next.delete(genre);else next.add(genre);setGf(next);}} style={{
              ...
              background:gf.has(genre)?color:"#0a0a10",
              color:gf.has(genre)?"#fff":"#333",
              border:`1px solid ${gf.has(genre)?color:"#222"}`,
              opacity:gf.has(genre)?1:0.4,
            }}>{genre} ({genreCounts[genre]||0})</button>
          ))}
        </div>
        <div style={{display:"flex",gap:4,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <span style={{color:"#2a2a35",margin:"0 4px"}}>|</span>
          ...
          <button onClick={()=>setTZoom(z=>Math.max(1,z-0.5))} style={{width:28,height:28,
            display:"flex",alignItems:"center",justifyContent:"center",
          ...
          <button onClick={()=>setTZoom(z=>Math.min(6,z+0.5))} style={{width:28,height:28,
            display:"flex",alignItems:"center",justifyContent:"center",
          ...
          <button onClick={()=>{setZoom(1);setTZoom(1);}} style={{padding:"3px 9px",
        ...
        <div style={{overflowX:"auto",position:"relative"}}>
          <div style={{minWidth:(1000+200*zoom)*tZoom}}>
            {/* Time ruler */}
            <div style={{display:"flex",marginLeft:200*zoom,marginBottom:2,position:"sticky",top:0,zIndex:5,background:"#08080d"}}>
              {tMarks.map((t,i)=>(
                <div key={t} style={{flex:1,fontSize:Math.min(13,10*zoom),
                  color:["12P","6P","12A"].includes(t)?"#888":"#3a3a44",
                  fontWeight:["12P","6P","12A"].includes(t)?700:400,
                  borderLeft:"1px solid #151520",paddingLeft:4,paddingBottom:2}}>
                  {t}
                </div>
              ))}
            </div>
            ...
            {/* Venue rows */}
            {venues.map(([venue,evts],vi)=>{
              // Hide venue if genre filter active and no events match
              if(!evts.some(e=>gf.has(e.genre))) return null;
              ...
              sortedEvts.forEach(evt=>{
                ...
                for(let r=0;r<rows.length;r++){
                  ...
                  if(eStart>=lEnd-0.05){rows[r].push(evt);placed=true;break;}
                }
              ...
              return(
                <div key={venue} style={{display:"flex",borderBottom:"1px solid #141420",minHeight:numRows*rowH+6,
                  ...
                  {/* Sticky venue label */}
                  <div style={{width:200,minWidth:200,padding:"5px 10px",borderRight:"2px solid #1e1e2a",
                    display:"flex",flexDirection:"column",justifyContent:"center",
                    position:"sticky",left:0,zIndex:3,
                    background:vi%2===0?"#0a0a12":"#0c0c15",
                    boxShadow:"4px 0 12px rgba(0,0,0,0.4)"}}>
                    <div style={{fontSize:11,fontWeight:700,color:"#ccc",lineHeight:1.3}}>{venue}</div>
                    <div style={{fontSize:9,color:"#4a4a55",marginTop:2,lineHeight:1.2,
                      whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:178}}>
                  ...
                  <div style={{flex:1,position:"relative",minHeight:numRows*rowH}}>
                    ...
                    {rows.map((row,ri)=>row.map(evt=>{
                      ...
                      return(
                        <div key={evt.id}
                          onClick={()=>toggleSched(evt.id)}
                          onMouseEnter={(e)=>{const r=e.currentTarget.getBoundingClientRect();setHov(evt.id);setHovRect(r);setExpanded(false);startExpand(evt);}}
                          onMouseLeave={()=>{stopExpand();
                          ...
                          style={{
                            ...
                            background:filt?"#14141c":isSaved?`linear-gradient(135deg,${color},${color}dd)`:`linear-gradient(135deg,${color}cc,${color}88)`,
                            borderRadius:3,
                          ...
                          {isSaved&&<span style={{fontSize:Math.min(12,rowH-6),lineHeight:1,flexShrink:0}}>★</span>}
                          <span style={{fontSize:Math.min(10+zoom,Math.max(7+zoom*0.5,width*0.9)),fontWeight:600,
                            ...
                            textShadow:filt?"none":"0 1px 2px rgba(0,0,0,.5)"}}>{evt.name}</span>
                        </div>
                      );
                    }))}
                  </div>
        ...
        {hov!==null&&(()=>{
          ...
          return(
            <div ref={panelRef}
              ...
              onMouseLeave={()=>{setHov(null);setExpanded(false);killAudio();}}
              style={{position:"fixed",left:px,top:Math.min(py,window.innerHeight-(expanded?420:200)),
              ...
              <div style={{maxHeight:expanded?500:0,overflow:"hidden",transition:"max-height .4s ease"}}>
                ...
                <div style={{padding:"10px 14px",background:"#0a0a14",display:"flex",alignItems:"center",gap:12,
                  ...
                  {audioState==="playing"?(
                    ...
                    <div style={{fontSize:11,color:"#666"}}>No preview available</div></>
                  ):(
                    <><span style={{fontSize:16,opacity:.5}}>🎵</span>
                    <div style={{fontSize:11,color:"#555"}}>Preview ended</div></>
                  )}
                </div>
                {/* Action buttons */}
        ...
        {/* Genre stats bar */}
        <div style={{marginTop:18,borderTop:"1px solid #1a1a22",paddingTop:14}}>
          <div style={{display:"flex",height:8,borderRadius:4,overflow:"hidden",background:"#111",marginBottom:10}}>
            {Object.entries(G).filter(([g])=>genreCounts[g]).sort((a,b)=>(genreCounts[b[0]]||0)-(genreCounts[a[0]]||0)).map(([genre,color])=>(
              <div key={genre} onClick={()=>{const next=new Set(gf);if(next.has(genre))next.delete(genre);else next.add(genre);setGf(next);}}
                style={{width:`${(genreCounts[genre]/dayEvts.length)*100}%`,background:color,cursor:"pointer",
                transition:"opacity .2s",opacity:!gf.has(genre)?0.2:1}} title={`${genre}: ${genreCounts[genre]}`}/>
            ))}
          </div>
          <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
            {Object.entries(G).filter(([g])=>genreCounts[g]).sort((a,b)=>(genreCounts[b[0]]||0)-(genreCounts[a[0]]||0)).map(([genre,color])=>(
              <div key={genre} onClick={()=>{const next=new Set(gf);if(next.has(genre))next.delete(genre);else next.add(genre);setGf(next);}} style={{
                padding:"5px 10px",background:gf.has(genre)?color+"22":"#0e0e16",borderRadius:5,cursor:"pointer",
                border:`1px solid ${color}${gf.has(genre)?"":"33"}`,display:"flex",alignItems:"center",gap:6,
                opacity:gf.has(genre)?1:0.35}}>
                <div style={{width:9,height:9,borderRadius:2,background:color,flexShrink:0}}/>
                <span style={{fontSize:11,fontWeight:600,color}}>{genre}</span>
                <span style={{fontSize:11,color:"#444"}}>{genreCounts[genre]}</span>
              </div>
            ))}
          </div>
        </div>

        <div style={{marginTop:12,fontSize:10,color:"#3a3a44",lineHeight:1.5}}>
        ...
        {viewMode==="mysched"&&(
          <div id="my-schedule-view">
            {mySched.size===0?(
              ...
              <div>
                {/* Controls */}
                <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16,flexWrap:"wrap"}} className="no-print">
                  <div style={{fontSize:18,fontWeight:700,color:"#FFB703"}}>★ My Schedule — {mySched.size} acts</div>
                  <button onClick={()=>{
                ...
                <div id="print-schedule">
                  <h1 style={{fontSize:22,fontWeight:800,color:"#fff",marginBottom:2}}>My SXSW 2026 Schedule</h1>
                  ...
                  {DAYS.map(d=>{
                    ...
                    return(
                      <div key={d} style={{marginBottom:20}}>
                        <h2 style={{fontSize:15,fontWeight:800,color:"#FFB703",marginBottom:8,
                          paddingBottom:4,borderBottom:"2px solid #FFB70333"}}>{DAY_LABEL[d]} — {dayEvts.length} acts</h2>
                        {dayEvts.map(evt=>{
                          const color=G[evt.genre]||"#555";
                          return(
                            <div key={evt.id} style={{display:"flex",gap:10,padding:"8px 4px",
                              borderBottom:"1px solid #1a1a22",alignItems:"center",flexWrap:"wrap"}}>
                              <div style={{minWidth:130,fontSize:11,fontWeight:700,color:"#aaa",fontVariantNumeric:"tabular-nums"}}>
                                {evt.time}
                              </div>
                              <div style={{minWidth:180,fontSize:13,fontWeight:700,color:"#fff"}}>{evt.name}</div>
                              <div style={{minWidth:160,fontSize:11,color:"#777"}}>{evt.venue}</div>
                              <span style={{fontSize:10,padding:"2px 8px",borderRadius:10,
                                background:color+"28",color:color,fontWeight:600}}>{evt.genre}</span>
                              {evt.presenter&&<span style={{fontSize:10,color:"#555",fontStyle:"italic"}}>{evt.presenter}</span>}
                              <button onClick={()=>toggleSched(evt.id)} className="no-print"
                                ...
                                fontSize:10,cursor:"pointer",fontFamily:"inherit",fontWeight:600}}>✕ Remove</button>
                            </div>
                          );
                        })}
                      </div>
